...
| `/abort` (or `/stop`) | Cancel the turn running on this chat and the background tasks and jobs it started, which otherwise outlive the turn. The reply says what it stopped; messages that arrived meanwhile run as a turn of their own. |
...
| `/cost` (or `/usage`) | What this session has cost over its whole log: tokens in, how many of them cached, tokens out, and the price where the model is priced. |
| `/cron` | The jobs scheduled for this chat, and the gateway's own: schedule, next run, id. `/cron remove <id\|name>` takes one away; a name shared by several jobs is refused in favour of the id. Adding is done by asking — the model has the `cron` tool. |
| `/tasks` | Subagents working for this chat, with agent, description and how long, and results held for delivery. |
| `/whoami` | Your sender id and the chat id as the channel reports them — what `allow_from` wants — and the session key. A stranger never sees it: a stranger is never answered. |
| `/restart` | Answer, then stop the way a signal would and exit with code 75, which the shipped service unit restarts on. |
| `/help` | The list above. |
